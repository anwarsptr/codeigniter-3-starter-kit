    ...
    <div class="col-md-4 col-md-offset-4">
      <div class="box box-primary border-radius">
        <div class="box-body box-profile" style="margin:10px 0px;">
          <form id="profile" action="javascript:postSubmit('profile', 'setup/webs/save')" method="post" data-parsley-validate="true">
            <div class="form-group has-feedback">
              <label for="app_name" class="control-label">App Name : <span class="text-danger">*</span></label>
              <input type="text" class="form-control border-radius" name="app_name" id="app_name" placeholder="Enter app name" value="<?= $query['app_name'] ?>" maxlength="100" required <?= $showPermission ? '':'readonly' ?>>
              <span class="fa fa-bookmark form-control-feedback"></span>
            </div>
            <div class="form-group has-feedback">
              <label for="title" class="control-label">Title : <span class="text-danger">*</span></label>
              <input type="text" class="form-control border-radius" name="title" id="title" placeholder="Enter title" value="<?= $query['title'] ?>" maxlength="100" required <?= $showPermission ? '':'readonly' ?>>
              <span class="fa fa-tag form-control-feedback"></span>
            </div>
            <div class="form-group has-feedback">
              <label for="descriptions" class="control-label">Description : </label>
              <textarea class="form-control border-radius custom-textarea" name="descriptions" id="descriptions" placeholder="Enter description" rows="1" <?= $showPermission ? '':'readonly' ?>><?= $query['description'] ?></textarea>
              <span class="fa fa-file-text form-control-feedback"></span>
            </div>
            <div class="form-group has-feedback">
              <label for="website" class="control-label">Website : </label>
              <input type="text" class="form-control border-radius" name="website" id="website" placeholder="domain.com" value="<?= $query['website'] ?>" style="text-transform:lowercase" <?= $showPermission ? '':'readonly' ?>>
          ...
          <?php if (!empty($query['updated_at'])): ?>
          <hr style="margin:10px 0px;">
          <div class="text-center">
              <b>Last Update :</b> <?= waktu($query['updated_at']) ?>
          </div>
          <?php endif; ?>
        </div>
      </div>
